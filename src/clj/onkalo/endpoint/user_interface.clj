...
(defn ui-routes [build-info {{translations :data} :translations :keys [session-store http-config] :as config}]
  (-> (routes
        (GET "/" []
      ...
      (wrap-session (merge {:store session-store}
                           (select-keys http-config [:cookie-attrs :cookie-name])))))
