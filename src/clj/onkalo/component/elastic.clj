...
;; DO NOT CHANGE THE VERSION UNLESS ABSOLUTELY NECESSARY
;; Reindexing in production takes 24+ hours and may be interrupted during that time.
;; Increasing the version causes automatic reindexing. Changing existing fields requires additional migration code.
(def index-version 29)
...
(defn- update-mapping! [c idx mapping]
  (s/request c {:url    [idx :_mapping]
                :method :put
...
(defn- document-count [c idx]
  (-> (s/request c {:url    [idx :_count]
                    :method :get})
      :body
      :count))
      ...
      (try
        (loop []
          (when-let [page (async/<! ch)]
            (cond
              (instance? Exception page)
              (throw (Exception. (str "Elasticsearch error: " (pr-str (ex-data page))) page))

              (not= 200 (:status page))
              (throw (Exception. (str "Elasticsearch error: " (pr-str (select-keys page [:status :body])))))

              (-> page :body :hits :hits nil?)
              (throw (Exception. (str "Elasticsearch response did not contain expected :hits key: " (pr-str (:body page)))))
        ...
        (let [old-count (document-count client old-index)
              new-count (document-count client new-index)]
          (if (< new-count old-count)
            (timbre/error "\uD83D\uDEA8 Reindexing ended in CATASTROPHIC FAILURE, new:" new-index new-count "docs, old:" old-index old-count "docs \uD83D\uDEA8")
            (timbre/info "Reindexing ready, new:" new-index new-count "docs, old:" old-index old-count "docs")))
        (catch Throwable t
