...
(defrecord OnkaloPubSub [project-id endpoint credentials-provider]
  ...
  (generate-preview [this gcs bucket id]
    (let [source-bucket  (os/org-bucket-name gcs bucket)
          preview-bucket (os/org-bucket-name gcs (os/org-bucket->org-preview-bucket bucket))]
      (pubsub/publish (:client this)
                      "to-conversion-service"
                      {:handler            :convert-to-jpeg
                       :message-id         (str source-bucket "/" id "-preview")
