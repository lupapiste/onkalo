(ns onkalo.util.auth
  (:require [clj-time.core :as t]
            ...
            [lupapiste-commons.ring.session-timeout :as commons-session-timeout]
...
(defn wrap-session-user-auth [handler]
  (fn [request]
    (let [archivist-orgs (->> request
      ...
      (if (and (not (commons-session-timeout/session-expired? request))
               (contains? archivist-orgs req-org))
        (handler request)
        {:status 403
