...
(defn allowed-document-types-search-parameter
  "Returns `:allowed-document-types` - list of organizatiion, types list
  map. `instance-type` must be either `:terminal` or `:departmental.`

  Sample return value:
  {:allowed-document-types [[\"123-R\" [\"muut.muu\"]] [\"001-R\" []]]}"
  [lupapiste-api instance-type organization-ids]
  (let [types-fn (case instance-type
                   :terminal     allowed-terminal-document-types-for-organization
                   :departmental allowed-departmental-document-types-for-organization)]
    {:allowed-document-types (map (fn [org-id]
                                    [org-id (or (types-fn lupapiste-api org-id) [])])
                                  organization-ids)}))
