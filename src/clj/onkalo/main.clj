(ns onkalo.main
  ...
  (:require [clojure.edn :as edn]
            ...
            [lupapiste-commons.gcp-json-logging :as gcp-json-logging]
...
(defn configure-logging [{:keys [format level]}]
  (if (= :json format)
    ;; Log JSON to stdout
    (do (tas/install {:inline-args?        true
                      ;; Also include ns, file, and line in logs
                      :should-log-field-fn (constantly true)
                      :msg-key             :message
                      :level-key           :severity
                      :key-names           {:err :exception}})
        (timbre/set-min-level! (or level :info))
        (timbre/merge-config! {:middleware [gcp-json-logging/combine-args-mw
                                            gcp-json-logging/string-stack-trace-mw]}))
    (timbre/set-config! {:level          (or level :info)
                         :ns-whitelist   []
                         :ns-blacklist   []
                         :middleware     []
