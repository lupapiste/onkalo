(ns onkalo.storage.document
  ...
  (:import [clojure.lang ExceptionInfo]
           ...
           [java.io CharConversionException File]
...
(def valid-content-types #{"application/pdf" "image/tiff" "text/xml" "application/xml" "image/jpeg"})
...
(defn- generate-preview
  ([config organization bucket id]
   (generate-preview config organization bucket id nil))
  ([{:keys [pubsub gcs]} organization bucket id content-type]
   (if (and content-type
            (contains? #{"text/xml" "application/xml"} content-type))
     (os/upload gcs (os/org-preview-bucket organization bucket) id {:content      (placeholder-image-is)
                                                                    :content-type "image/jpeg"})
     (preview-boundary/generate-preview pubsub gcs bucket id))))

(defn- upload-valid-doc! [{{:keys [bucket-suffix]} :storage :keys [elastic gcs] :as config}
                          organization
                          id
                          overwrite
                          ^File tempfile
  ...
  (let [bucket                                          (os/org-bucket organization bucket-suffix)
        {doc-bucket :bucket file-id :fileId :as es-doc} (ea/get-document elastic bucket id nil true)
        object-exists?                                  (and es-doc
                                                             (os/object-exists? gcs doc-bucket file-id))
        ...
        target-bucket                                   (or doc-bucket bucket)
        file-id                                         (or file-id id)]
    (if (or (not object-exists?)
            (-> es-doc :metadata :deleted)
            overwrite)
      (try
        (xmp/write-metadata-to-file! tempfile updated-metadata content-type)
        (os/upload storage target-bucket file-id {:content      tempfile
                                                  :content-type content-type})
        (ea/persist-document-to-index elastic
                                      target-bucket
                                      (ea/es-id bucket file-id)
                                      file-id
                                      (md-util/sha256 tempfile)
                                      content-type
                                      (os/default-storage-id config)
                                      updated-metadata)
        (generate-preview config organization target-bucket file-id content-type)
        (response "OK")
        ...
        (catch SAXParseException ex
          ...
          (if (instance? CharConversionException (.getCause ex))
        ...
        (catch CharConversionException ex ; pre java 11
          (timbre/warn ex "Could not parse XMP metadata in"
                       (:tiedostonimi updated-metadata) "id" id "from application" (:applicationId updated-metadata))
          (bad-request (str "XMP metadata embedded in the file is not valid UTF-8: " (.getMessage ex))))
...
(def content-types-to-verify #{"image/jpeg" "image/tiff"})

(def generate-temp-id (comp str random-uuid))

(defn invalid-contents?
  [{:keys [laundry] :as config} file content-type]
  (let [storage     (os/default-storage config)
        temp-bucket (os/temp-bucket-name storage)
        id          (generate-temp-id)]
    (try
      ;; Temporarily store file in a bucket so that Laundry may access it
      (os/upload-temp-file storage id {:content      file
                                       :content-type content-type})
      (try
        (not (laundry-api/verify-image-archivable!
               laundry
               {:bucket                temp-bucket
                :object-id             id
        ...
        (finally
          ;; Cleanup
          (os/delete-temp-file storage id)))
      (catch Exception e
        (timbre/error e "Failed to verify image contents")
        true))))
  ...
  (try
    (let [{:keys [organization tosFunction type]
      ...
      (cond
        ...
        (and (content-types-to-verify content-type)
             (invalid-contents?
               config tempfile content-type)) (bad-request {:content "Content-type does not match file contents"})
...
(defn- get-es-doc
  [{{:keys [bucket-suffix]} :storage :keys [elastic]} organization id]
  (let [bucket (os/org-bucket organization bucket-suffix)
        es-doc (ea/get-document elastic bucket id nil true)
        _      (when-not es-doc
...
(defn- get-doc-from-bucket
  ...
  (if-not organization
    (-> (response {:error "Missing organization"})
        (status 400))
    (try
      (let [{doc-bucket :bucket file-id :fileId :as es-doc} (get-es-doc config organization id)
            storage                                         gcs
            target-bucket                                   (if preview?
                                                              (os/org-bucket->org-preview-bucket doc-bucket)
                                                              doc-bucket)
            file-data                                       (os/download storage target-bucket file-id)]
...
(defn get-document
  ...
  ([config id organization http-response-format]
   (get-doc-from-bucket config id organization false http-response-format)))

(defn get-preview [config id organization]
  (let [resp (get-doc-from-bucket config id organization true :inline)]
    (if (= 404 (:status resp))
      ;; Add missing image to preview generation queue
      (let [{doc-bucket :bucket file-id :fileId} (get-es-doc config organization id)]
        (generate-preview config organization doc-bucket file-id)
        {:status          200
...
(defn document-metadata
  ...
  ([{{:keys [bucket-suffix]} :storage :keys [elastic gcs] :as config} id organization exists-only? source-fields]
   (if-not organization
     (-> (response {:error "Missing organization"})
         (status 400))
     (let [bucket                  (os/org-bucket organization bucket-suffix)
           {storage-bucket :bucket
            file-id        :fileId
            :as            es-doc} (ea/get-document elastic bucket id source-fields true)]
       (if (and es-doc
                (os/object-exists? gcs storage-bucket file-id))
         {:status 200
          :body   (if exists-only?
                    {:exists true}
                    (dissoc es-doc :storage :bucket))}
         {:status 404
          :body   {:exists false}})))))
