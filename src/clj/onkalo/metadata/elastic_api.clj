...
(defn persist-document-to-index [elastic bucket es-id file-id sha256-sum content-type storage {:keys [organization] :as metadata}]
  (let [document  (->> {:source       "lupapiste"
                        :bucket       bucket
                        :fileId       file-id
                        :sha256       sha256-sum
...
(defn get-document
  ...
  ([elastic bucket id source-fields]
   (get-document elastic bucket id source-fields false))
  ([elastic bucket id source-fields include-bucket?]
   (let [source-fields (when (some? source-fields)
                         (conj source-fields "storage" "bucket" "fileId"))
         doc-id        (es-id bucket id)]
     (or (cond-> (esd/get-by-id elastic doc-id source-fields)
           true                  :body
           true                  :_source
           (not include-bucket?) (dissoc :bucket))
         ;; Tackle the cases where some original documents might not have the file id as their es document id
         (when-let [doc (->> (find-from-es elastic
                                           ...
                                           1
                                           source-fields
                                           (if include-bucket? :_source source-without-bucket))
                             :results
                             first)]
           (timbre/debugf "Found %s by bucket and fileId" doc-id)
           doc)))))

(defn delete-document [elastic bucket id]
  (esd/delete elastic (es-id bucket id)))
