...
(defn- with-retention-period
  [{{:keys [pituus]} :sailytysaika
    :keys            [paatospvm] :as metadata} id]
  (if (and paatospvm pituus)
    (assoc-in metadata [:sailytysaika :retention-period-end] (calculate-end-date paatospvm pituus))
    (do
      ;; TODO: We should detect & handle this sooner, LPK-6893
      (timbre/warn "Cannot calculate retention period!" id paatospvm pituus)
      metadata)))

(defn- with-security-period
  [{:keys [salassapitoaika paatospvm] :as metadata} id]
  (if (and paatospvm salassapitoaika)
    (assoc metadata :security-period-end (calculate-end-date paatospvm salassapitoaika))
    (do
      ;; TODO: We should detect & handle this sooner, LPK-6893
      (timbre/warn "Cannot calculate security period!" id paatospvm salassapitoaika)
      metadata)))

(defn- update-end-dates [id {{:keys [arkistointi]} :sailytysaika
                             :keys                 [julkisuusluokka] :as metadata}]
  (cond-> metadata
    (= arkistointi :määräajan)       (with-retention-period id)
    (not= julkisuusluokka :julkinen) (with-security-period id)))
...
(defn update-metadata [{{:keys [bucket-suffix]} :storage :keys [elastic gcs] :as config}
                       ^String id
                       new-metadata
                       user
                       organization]
  (try
    (let [bucket (os/org-bucket organization bucket-suffix)]
      (if-let [{doc-bucket :bucket file-id :fileId :as old-data} (ea/get-document elastic bucket id nil true)]
        (let [old-metadata      (->> (-> (:metadata old-data)
          ...
          (if (not= old-metadata merged-metadata)
            (let [updated-metadata       (->> merged-metadata
                  ...
                  {:keys [content-fn
                          content-type]} (os/download gcs doc-bucket file-id)
              ...
              (ea/persist-document-to-index elastic
                                            ...
                                            updated-metadata)
              (os/upload new-storage doc-bucket file-id {:content      tempfile
