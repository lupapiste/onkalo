(ns onkalo.metadata.xmp-processor
  "Tools for embedding and extracting Onkalo metadata from files.
  Embedded metadata acts as a backup for the original metadata in ES."
  ...
  (:import [com.lowagie.text.pdf PdfReader PdfStamper]
           [java.io ByteArrayInputStream CharConversionException File InputStream ByteArrayOutputStream]
...
(defn- generate-merged-rdf ^String [parsed-xmp-xml metadata]
...
(defn- add-metadata-to-pdf-file [^File file reader ^String rdf-xml]
...
(defn- write-updated-tiff-to-output-stream! [^String xml-str ^TiffImageMetadata tiff-metadata metadata output-stream]
...
(defn write-metadata-to-file! [^File file metadata content-type]
  (case content-type
    ...
    "image/jpeg" file ; No metadata into JPEGs
    file))
...
(defn remove-metadata ^InputStream [^InputStream is content-type]
  (case content-type
    "application/pdf" (remove-metadata-from-pdf is)
    "image/tiff" (remove-metadata-from-tiff is)
    ;; We don't embed metadata in JPEGs
    "image/jpeg" is))
