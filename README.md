...
Onkalo uses Laundry to verify archived images. But only images, not PDF/A files. When running Onkalo locally against a Laundry instance running within GCP, you need to add the IAP client credentials to `config-local.edn`. This is not needed if you also run a local Laundry instance.

```clojure
:laundry {:host                 "https://laundry.dev.fi.cloudpermit.com"
          :iap-oauth-client     true
          :iap-oauth-client-id  "<client id>.apps.googleusercontent.com"
          :service-account-file "/path/to/credentials_file.json"}
```

### Using Docker

By default, running `docker-compose up` starts the elasticsearch container.
...
Note that npm dependencies required by the shared code in document-search-commons must be declared in onkalo project's
package.json as that project only includes CLJS libraries loaded via Leiningen.

You should symlink document-search-commons under checkouts for front-end development, as shadow-cljs expects to access
sources from there as well. Styles are also located under document-search-commons. Run sass in *document-search-commons* 
for continuous css build when altering styles:
